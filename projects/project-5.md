...
Schedule Page
Included thumbnail images with popups, hyperlinks, slideshow
pulls from database
google translate
distance with map
...
Form Input Page
Allows entry addition into schedule database
basic validation (alphnumeric, date/time ranges)
page navigation, successful submission

Widgets Page
twitter stream, instagram widget, password strength validator (used jquery and javascript)

All styled with css (both inline, embedded, and external)

Back-End
HTTP requests - GET and POST
establish socket and binds to a port
send request to server, server handles request and returns appropriate response
implemented a server different ways - HTTP protocol and Express (Node.js), 

Used SQL/MySQL to create, push, and pull from databased
Created Admin page to allow login and sessions
